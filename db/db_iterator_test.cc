...
TEST_P(DBIteratorTest, IteratorProperty_WithTimestamp) {
  ...
  CreateAndReopenWithCF({"pikachu"}, options);
  WriteOptions wo;
  wo.asif_commit_timestamps = {999999};
  Put(1, "1", "2", wo);
  Delete(1, "2", wo);
  ReadOptions ropt;
  ropt.read_timestamp = 9999999;
  ropt.pin_data = false;
  {
...
TEST_P(DBIteratorTest, NonBlockingIteration_WithTimestamp) {
  do {
    ...
    options.statistics = rocksdb::CreateDBStatistics();
    non_blocking_opts.read_tier = kBlockCacheTier;
    non_blocking_opts.read_timestamp = 999999999999;
    regular_opts.read_timestamp = 9999999999;

    WriteOptions wo;
    wo.asif_commit_timestamps = {1111111111};
    CreateAndReopenWithCF({"pikachu"}, options);
    // write one kv to the database.
    ASSERT_OK(Put(1, "a", "b", wo));
    ...
    // read in the specified block via a regular get
    ASSERT_EQ(Get(1, "a", regular_opts), "b");
...
TEST_P(DBIteratorTest, IterSeekBeforePrevWithTimestamp) {
  ...
  // snapshots[0] null
  // snapshots[1] a,c   commit time 1000
  // snapshots[2] 0,1,f commit time 2000
  // snapshots[3] 2     commit time 3000

  snapshots.emplace_back(db_->GetSnapshot());
  {
    WriteOptions wo;
    wo.asif_commit_timestamps = {1000};
    ASSERT_OK(Put("a", "b", wo));
    ASSERT_OK(Put("c", "d", wo));
    dbfull()->Flush(FlushOptions());
    snapshots.emplace_back(db_->GetSnapshot());

    wo.asif_commit_timestamps = {2000};
    ASSERT_OK(Put("0", "f", wo));
    ASSERT_OK(Put("1", "h", wo));
    ASSERT_OK(Put("f", "e", wo));
    snapshots.emplace_back(db_->GetSnapshot());

    wo.asif_commit_timestamps = {3000};
    dbfull()->Flush(FlushOptions());
    ASSERT_OK(Put("2", "j", wo));
    snapshots.emplace_back(db_->GetSnapshot());

    // read < commit, can't see any
    ReadOptions ro;
    ...
    // read <= commit 1000, can't see commit > 1000
    ...
    iter->Seek(Slice("1"));
    // can't find `1`, but can see `a`,  `c`, `a` is the past
    ASSERT_EQ(IterStatus(iter), "a->b");
    iter->Seek(Slice("b"));
    // can't find `1`, but can see `a`,  `c`, `c` is the past
    ASSERT_EQ(IterStatus(iter), "c->d");
    delete iter;

    // snapshots[0] which is empty, can't see any
    ro.snapshot = snapshots[0];
    iter = NewIterator(ro);
    iter->Seek(Slice("c"));
    ASSERT_EQ(IterStatus(iter), "(invalid)");
    ...
    // read <= commit 2000, can't see commit > 2000
    ...
    ASSERT_EQ(IterStatus(iter), "a->b");
    iter->Prev();
    ASSERT_EQ(IterStatus(iter), "1->h");

    iter->Seek(Slice("0"));
    ASSERT_EQ(IterStatus(iter), "0->f");
    iter->Seek(Slice("1"));
    ASSERT_EQ(IterStatus(iter), "1->h");
    iter->Prev();
    ASSERT_EQ(IterStatus(iter), "0->f");

    // can't find `2`, but can see `0`, `1`, `a`, `c`, `f`, `a` is the past
    iter->Seek(Slice("2"));
    ASSERT_EQ(IterStatus(iter), "a->b");

    iter->Seek(Slice("e"));
    ASSERT_EQ(IterStatus(iter), "f->e");
    delete iter;
...
TEST_P(DBIteratorTest, IterSeekForPrevBeforeNextWithTimestamp) {
  ...
  snapshots.emplace_back(db_->GetSnapshot());
  {
    ...
    // can't find `1`, but can see `a`,  `c`, nothing before `a`, which is
    // invalid
    ASSERT_EQ(IterStatus(iter), "(invalid)");
    iter->SeekForPrev(Slice("b"));
    // can't find `b`, but can see `a`,  `c`, `a` is the last
    ASSERT_EQ(IterStatus(iter), "a->b");
    delete iter;
    ...
    // can't find `2`, but can see `0`, `1`, `a`, `c`, `f`, `1` is the last
    iter->SeekForPrev(Slice("2"));
    ASSERT_EQ(IterStatus(iter), "1->h");

    iter->SeekForPrev(Slice("e"));
    ASSERT_EQ(IterStatus(iter), "c->d");
    delete iter;
...
TEST_P(DBIteratorTest, IterLongKeys_WithTimestamp) {
  ...
  ASSERT_OK(Put(MakeLongKey(20, 0), "0", wo));
  ASSERT_OK(Put(MakeLongKey(32, 2), "2", wo));
  ASSERT_OK(Put("a", "b", wo));
  dbfull()->Flush(FlushOptions());
  ASSERT_OK(Put(MakeLongKey(50, 1), "1", wo));
  ASSERT_OK(Put(MakeLongKey(127, 3), "3", wo));
  ASSERT_OK(Put(MakeLongKey(64, 4), "4", wo));
  auto iter = NewIterator(ro);
  ...
  ro.read_timestamp = 20;
  iter = NewIterator(ro);
  iter->Seek(MakeLongKey(50, 1));
  ASSERT_EQ(IterStatus(iter), "(invalid)");
  // assert valid_ failed
  // iter->Next();
  // ASSERT_EQ(IterStatus(iter), "(invalid)");
  delete iter;
}
...
TEST_P(DBIteratorTest, IterNextWithNewerSeqWithTimestamp) {
  ...
  WriteOptions wo;
  wo.asif_commit_timestamps = {2000};
  ASSERT_OK(Put("0", "0", wo));
  dbfull()->Flush(FlushOptions());
  ASSERT_OK(Put("a", "b", wo));
  ASSERT_OK(Put("c", "d", wo));
  ASSERT_OK(Put("d", "e", wo));
  auto iter = NewIterator(ro);
  wo.asif_commit_timestamps = {1000};
  // Create a key that needs to be skipped for Seq too new
  for (uint64_t i = 0; i < last_options_.max_sequential_skip_in_iterations + 1;
       i++) {
    ASSERT_OK(Put("b", "f", wo));
  }
...
TEST_P(DBIteratorTest, IterPrevWithNewerSeq_WithTimestamp) {
  ...
  WriteOptions wo;
  wo.asif_commit_timestamps = {22222222222};
  ASSERT_OK(Put("0", "0", wo));
  dbfull()->Flush(FlushOptions());
  ASSERT_OK(Put("a", "b", wo));
  ASSERT_OK(Put("c", "d", wo));
...
TEST_P(DBIteratorTest, IterPrevWithNewerSeq2_WithTimestamp) {
  ...
  ASSERT_OK(Put("a", "b", wo));
  ASSERT_OK(Put("c", "d", wo));
  ASSERT_OK(Put("e", "f", wo));
  wo.asif_commit_timestamps = {1000};

  auto iter = NewIterator(ro);
  auto iter2 = NewIterator(ro);
  iter->Seek(Slice("c"));
  iter2->SeekForPrev(Slice("d"));
  ASSERT_EQ(IterStatus(iter), "c->d");
  ASSERT_EQ(IterStatus(iter2), "c->d");
...
TEST_P(DBIteratorTest, IterEmptyWithTimestamp) {
  ...
  do {
    CreateAndReopenWithCF({"pikachu"}, CurrentOptions());
    Iterator* iter = NewIterator(ro, handles_[1]);
...
TEST_P(DBIteratorTest, IterMultiWithTimestamp) {
  ...
  do {
    ...
    CreateAndReopenWithCF({"pikachu"}, CurrentOptions());
    ASSERT_OK(Put(1, "a", "va", wo));
    ASSERT_OK(Put(1, "b", "vb", wo));
    ASSERT_OK(Put(1, "c", "vc", wo));
    Iterator* iter = NewIterator(ro, handles_[1]);
    ...
    delete iter;
    ro.read_timestamp = 20;
    iter = NewIterator(ro, handles_[1]);
    iter->SeekToFirst();
    ASSERT_EQ(IterStatus(iter), "a->va2");
    iter->Next();
    ASSERT_EQ(IterStatus(iter), "a2->va3");
...
TEST_P(DBIteratorTest, IterReseekWithTimestamp) {
  ...
  ro.read_timestamp = 1000;
  WriteOptions wo;
  wo.asif_commit_timestamps = {500};
  // insert three keys with same userkey and verify that
  // reseek is not invoked. For each of these test cases,
  // verify that we can find the next key "b".
  ASSERT_OK(Put(1, "a", "zero", wo));
  ASSERT_OK(Put(1, "a", "one", wo));
  ASSERT_OK(Put(1, "a", "two", wo));
  ASSERT_OK(Put(1, "b", "bone", wo));
  Iterator* iter = NewIterator(ro, handles_[1]);
  iter->SeekToFirst();
  ...
  ASSERT_EQ(TestGetTickerCount(options, NUMBER_OF_RESEEKS_IN_ITERATION), 0);
  iter->Next();
  // in USE_TIMESTAMPS mode, reseek is disabled
  ASSERT_EQ(TestGetTickerCount(options, NUMBER_OF_RESEEKS_IN_ITERATION), 0);
  ASSERT_EQ(IterStatus(iter), "b->bone");
  delete iter;
  ...
  // Noticed: timestamp must be specific
  ASSERT_OK(Put(1, "b", "btwo", wo));
  iter = NewIterator(ro, handles_[1]);
  iter->SeekToLast();
  ...
  ASSERT_EQ(TestGetTickerCount(options, NUMBER_OF_RESEEKS_IN_ITERATION),
            num_reseeks);
  iter->Prev();
  // USE_TIMESTAMPS mode does not reseek
  ASSERT_EQ(TestGetTickerCount(options, NUMBER_OF_RESEEKS_IN_ITERATION),
            num_reseeks);
  ASSERT_EQ(IterStatus(iter), "a->four");
  delete iter;
  ...
  ASSERT_OK(Put(1, "b", "bthree", wo));
  ASSERT_OK(Put(1, "b", "bfour", wo));
  iter = NewIterator(ro, handles_[1]);
  iter->SeekToLast();
...
TEST_P(DBIteratorTest, IterReseekWithTimestamp1) {
  anon::OptionsOverride options_override;
  options_override.skip_policy = kSkipNoSnapshot;
  Options options = CurrentOptions(options_override);
  options.max_sequential_skip_in_iterations = 3;
  ...
  // reseek is not invoked. For each of these test cases,
  // verify that we can find the next key "b".
  ASSERT_OK(Put(1, "a", "zero", wo));  // (x,  500)
  ASSERT_OK(Put(1, "a", "one", wo));   // (x+1,500)
  ASSERT_OK(Put(1, "a", "two", wo));   // (x+2,500)
  wo.asif_commit_timestamps = {400};
  ASSERT_OK(Put(1, "b", "bone", wo));             // (x+3,400)
  Iterator* iter = NewIterator(ro, handles_[1]);  // seq x+3
  iter->SeekToFirst();
  ASSERT_EQ(TestGetTickerCount(options, NUMBER_OF_RESEEKS_IN_ITERATION), 0);
  ASSERT_EQ(IterStatus(iter), "a->two");
  iter->Next();
  ...
  wo.asif_commit_timestamps = {600};
  ASSERT_OK(Put(1, "a", "three", wo));  // (x+4,600)
  ASSERT_OK(Put(1, "a", "four", wo));   // (x+5,600)
  ro.read_timestamp = 400;
  iter = NewIterator(ro, handles_[1]);  // seq x+5
  iter->SeekToFirst();
  // in USE_TIMESTAMPS mode, reseek is disabled
  ASSERT_EQ(TestGetTickerCount(options, NUMBER_OF_RESEEKS_IN_ITERATION), 0);
  ASSERT_EQ(IterStatus(iter), "b->bone");
...
TEST_P(DBIteratorTest, IterSmallAndLargeMix_WithTimestamp) {
  do {
    ...
    ASSERT_OK(Put(1, "a", "va", wo));
    wo.asif_commit_timestamps = {2000};
    ASSERT_OK(Put(1, "b", std::string(100000, 'b'), wo));
    ASSERT_OK(Put(1, "c", "vc", wo));
    wo.asif_commit_timestamps = {3000};
    ASSERT_OK(Put(1, "d", std::string(100000, 'd'), wo));
    ASSERT_OK(Put(1, "e", std::string(100000, 'e'), wo));
    ReadOptions ro;

    ro.read_timestamp = 4000;
    Iterator* iter = NewIterator(ro, handles_[1]);
...
TEST_P(DBIteratorTest, IterMultiWithDelete_WithTimestamp) {
  do {
    ...
    ro.read_timestamp = 4000;
    CreateAndReopenWithCF({"pikachu"}, CurrentOptions());
    ASSERT_OK(Put(1, "ka", "va", wo));
    ASSERT_OK(Put(1, "kb", "vb", wo));
    ASSERT_OK(Put(1, "kc", "vc", wo));
    ASSERT_OK(Delete(1, "kb"));
    ASSERT_EQ("NOT_FOUND", Get(1, "kb"));
...
TEST_P(DBIteratorTest, IterPrevMaxSkip_WithTimestamp) {
  ...
  do {
    CreateAndReopenWithCF({"pikachu"}, CurrentOptions());
    for (int i = 0; i < 2; i++) {
      ASSERT_OK(Put(1, "key1", "v1", wo));
      ASSERT_OK(Put(1, "key2", "v2", wo));
      ASSERT_OK(Put(1, "key3", "v3", wo));
      ASSERT_OK(Put(1, "key4", "v4", wo));
      ASSERT_OK(Put(1, "key5", "v5", wo));
      wo.asif_commit_timestamps[0] += 500;
    }
    ro.read_timestamp = 1000000;
    VerifyIterLast("key5->v5", ro, 1);

    ASSERT_OK(Delete(1, "key5", wo));
    VerifyIterLast("key4->v4", ro, 1);
    VerifyIterLast("(invalid)", 1);

    ASSERT_OK(Delete(1, "key4", wo));
    VerifyIterLast("key3->v3", ro, 1);

    ASSERT_OK(Delete(1, "key3", wo));
    VerifyIterLast("key2->v2", ro, 1);

    ASSERT_OK(Delete(1, "key2", wo));
    VerifyIterLast("key1->v1", ro, 1);

    ASSERT_OK(Delete(1, "key1", wo));
...
TEST_P(DBIteratorTest, IterWithSnapshot_WithTimestamp) {
  ...
  WriteOptions wo;
  do {
    options.read_timestamp = 1111111111111111;
    wo.asif_commit_timestamps = {1111111111};
    CreateAndReopenWithCF({"pikachu"}, CurrentOptions(options_override));
    ASSERT_OK(Put(1, "key1", "val1", wo));
    ASSERT_OK(Put(1, "key2", "val2", wo));
    ASSERT_OK(Put(1, "key3", "val3", wo));
    ASSERT_OK(Put(1, "key4", "val4", wo));
    ASSERT_OK(Put(1, "key5", "val5", wo));
    ...
    ASSERT_OK(Put(1, "key0", "val0", wo));
    // Put more values after the snapshot
    ASSERT_OK(Put(1, "key100", "val100", wo));
    ASSERT_OK(Put(1, "key101", "val101", wo));
    ...
    delete iter;
    options.read_timestamp = 111;
    iter = NewIterator(options, handles_[1]);
    iter->Seek("key5");
    ASSERT_TRUE(!iter->Valid());
    db_->ReleaseSnapshot(snapshot);
    delete iter;
...
TEST_P(DBIteratorTest, IteratorPinsRef_WithTimestamp) {
  do {
    CreateAndReopenWithCF({"pikachu"}, CurrentOptions());
    WriteOptions wo;
    wo.asif_commit_timestamps = {10000000};
    Put(1, "foo", "hello", wo);
    ReadOptions ro;
    ro.read_timestamp = 10000000000;
    // Get iterator that will yield the current contents of the DB.
    Iterator* iter = NewIterator(ro, handles_[1]);

    // Write to force compactions
    wo.asif_commit_timestamps[0]++;
    Put(1, "foo", "newvalue1", wo);
    for (int i = 0; i < 100; i++) {
      // 100K values
      wo.asif_commit_timestamps[0]++;
      ASSERT_OK(Put(1, Key(i), Key(i) + std::string(100000, 'v'), wo));
    }
    wo.asif_commit_timestamps[0]++;
    Put(1, "foo", "newvalue2", wo);
...
TEST_P(DBIteratorTest, DBIteratorBoundTest_WithTimestamp) {
  ...
  wo.asif_commit_timestamps = {100000};
  DestroyAndReopen(options);
  ASSERT_OK(Put("a", "0", wo));
  ASSERT_OK(Put("foo", "bar", wo));
  ASSERT_OK(Put("foo1", "bar1", wo));
  ASSERT_OK(Put("g1", "0", wo));
  ...
  wo.asif_commit_timestamps = {1000000};
  DestroyAndReopen(options);
  ASSERT_OK(Put("a", "0", wo));
  ASSERT_OK(Put("foo", "bar", wo));
  ASSERT_OK(Put("foo1", "bar1", wo));
  ...
  // if the bound has already reached
  {
    ...
    DestroyAndReopen(options);
    ASSERT_OK(Put("a", "0", wo));
    ASSERT_OK(Put("b", "0", wo));
    ASSERT_OK(Put("b1", "0", wo));
    ASSERT_OK(Put("c", "0", wo));
    ASSERT_OK(Put("d", "0", wo));
    ASSERT_OK(Put("e", "0", wo));
    ASSERT_OK(db_->Delete(wo, "c"));
    ASSERT_OK(db_->Delete(wo, "d"));
...
TEST_P(DBIteratorTest, DBIteratorBoundOptimizationTest_WithTimestamp) {
  ...
  wo.asif_commit_timestamps = {100};
  ASSERT_OK(Put("foo1", "bar1", wo));
  ASSERT_OK(Put("foo2", "bar2", wo));
  ASSERT_OK(Put("foo4", "bar4", wo));
  ASSERT_OK(Flush());
...
TEST_P(DBIteratorTest, PrevAfterAndNextAfterMerge_WithTimestamp) {
  ...
  // write three entries with different keys using Merge()
  WriteOptions wopts;
  wopts.asif_commit_timestamps = {10};
  db_->Merge(wopts, "1", "data1");
  db_->Merge(wopts, "2", "data2");
  db_->Merge(wopts, "3", "data3");
  ReadOptions ro;
  ro.read_timestamp = 11;
  std::unique_ptr<Iterator> it(NewIterator(ro));
...
TEST_P(DBIteratorTest, PinnedDataIteratorMultipleFiles_WithTimestamp) {
  ...
  ReadOptions ro;
  ro.pin_data = true;
  ro.read_timestamp = wo.asif_commit_timestamps[0] + 1;
  std::unique_ptr<Iterator> iter(NewIterator(ro));
...
TEST_P(DBIteratorTest, PinnedDataIteratorReadAfterUpdate_WithTimestamp) {
  ...
  for (auto& kv : true_data) {
    if (rnd.OneIn(2)) {
      ASSERT_OK(db_->Delete(wo, kv.first));
    } else {
      std::string new_val = RandomString(&rnd, 1000);
      ASSERT_OK(Put(kv.first, new_val, wo));
    }
  }
...
TEST_P(DBIteratorTest, IterSeekForPrevCrossingFiles_WithTimestamp) {
  Options options = CurrentOptions();
  options.prefix_extractor.reset(NewFixedPrefixTransform(1));
  options.disable_auto_compactions = true;
  // Enable prefix bloom for SST files
  ...
  WriteOptions wo;
  wo.asif_commit_timestamps = {100};
  ASSERT_OK(Put("a1", "va1", wo));
  ASSERT_OK(Put("a2", "va2", wo));
  ASSERT_OK(Put("a3", "va3", wo));
  ASSERT_OK(Flush());

  ASSERT_OK(Put("b1", "vb1", wo));
  ASSERT_OK(Put("b2", "vb2", wo));
  ASSERT_OK(Put("b3", "vb3", wo));
  ASSERT_OK(Flush());

  ASSERT_OK(Put("b4", "vb4", wo));
  ASSERT_OK(Put("d1", "vd1", wo));
  ASSERT_OK(Put("d2", "vd2", wo));
  ASSERT_OK(Put("d4", "vd4", wo));
  ASSERT_OK(Flush());
...
TEST_P(DBIteratorTest, IterPrevKeyCrossingBlocks_Withtimestamp) {
  ...
  // Putting such deletes will force DBIter::Prev() to fallback to a Seek
  for (int file_num = 0; file_num < 10; file_num++) {
    ASSERT_OK(db_->Delete(wo, "key4"));
    ASSERT_OK(Flush());
  }

  // First File containing 5 blocks of puts
  ASSERT_OK(Put("key1", "val1.0", wo));
  ASSERT_OK(Put("key2", "val2.0", wo));
  ASSERT_OK(Put("key3", "val3.0", wo));
  ASSERT_OK(Put("key4", "val4.0", wo));
  ASSERT_OK(Put("key5", "val5.0", wo));
  ASSERT_OK(Flush());

  // Second file containing 9 blocks of merge operands
  ASSERT_OK(db_->Merge(wo, "key1", "val1.1"));
  ASSERT_OK(db_->Merge(wo, "key1", "val1.2"));

  ASSERT_OK(db_->Merge(wo, "key2", "val2.1"));
  ASSERT_OK(db_->Merge(wo, "key2", "val2.2"));
  ASSERT_OK(db_->Merge(wo, "key2", "val2.3"));

  ASSERT_OK(db_->Merge(wo, "key3", "val3.1"));
  ASSERT_OK(db_->Merge(wo, "key3", "val3.2"));
  ASSERT_OK(db_->Merge(wo, "key3", "val3.3"));
  ASSERT_OK(db_->Merge(wo, "key3", "val3.4"));
  ASSERT_OK(Flush());
...
TEST_P(DBIteratorTest, IterPrevKeyCrossingBlocksRandomized_WithTimestamp) {
  ...
  for (int i = 0; i < kNumKeys; i++) {
    ...
    gen_val = RandomString(&rnd, kValSize);
    wo.asif_commit_timestamps[0]++;
    ASSERT_OK(Put(gen_key, gen_val, wo));
    true_data[gen_key] = gen_val;
  }
  ASSERT_OK(Flush());
  ...
  for (int i = 0; i < kNumKeys; i++) {
    ...
    for (int j = 0; j < kNumMergeOperands; j++) {
      ...
      ASSERT_OK(db_->Merge(wo, gen_key, gen_val));
      true_data[gen_key] += "," + gen_val;
    }
  }
  ASSERT_OK(Flush());
  ...
  for (int i = 0; i < kNumKeys; i++) {
    if (rnd.OneIn(static_cast<int>(100.0 / kDeletePercentage))) {
      ...
      ASSERT_OK(db_->Delete(wo, gen_key));
      true_data.erase(gen_key);
    }
  }
  ASSERT_OK(Flush());
...
TEST_P(DBIteratorTest, ReadAhead_WithTimestamp) {
  ...
  options.statistics->setTickerCount(NO_FILE_OPENS, 0);
  ReadOptions read_options;
  read_options.read_timestamp = 302;
  auto* iter = NewIterator(read_options);
  iter->SeekToFirst();
  int64_t num_file_opens = TestGetTickerCount(options, NO_FILE_OPENS);
  size_t bytes_read = env_->random_read_bytes_counter_;
...
TEST_P(DBIteratorTest, DBIteratorSkipRecentDuplicatesTest_WithTimestamp) {
  ...
  for (int i = 0; i < 100; ++i) {
    wo.asif_commit_timestamps[0]++;
    ASSERT_OK(Put("b", std::to_string(i + 1).c_str(), wo));
  }
  ...
  EXPECT_EQ(get_perf_context()->internal_recent_skipped_count, 100);
...
TEST_P(DBIteratorTest, Refresh_WithTimestamp) {
  ...
  ASSERT_OK(Put("x", "y", wo));
  ReadOptions ro;
  ro.read_timestamp = 10;
  std::unique_ptr<Iterator> iter(NewIterator(ro));
  iter->Seek(Slice("a"));
  ASSERT_TRUE(iter->Valid());
  ASSERT_EQ(iter->key().compare(Slice("x")), 0);
...
TEST_P(DBIteratorTest, UpperBoundWithChangeDirection_WithTimestamp) {
  ...
  wo.asif_commit_timestamps = {1000};
  // write a bunch of kvs to the database.
  ASSERT_OK(Put("a", "1", wo));
  ASSERT_OK(Put("y", "1", wo));
  ASSERT_OK(Put("y1", "1", wo));
  ASSERT_OK(Put("y2", "1", wo));
  ASSERT_OK(Put("y3", "1", wo));
  ASSERT_OK(Put("z", "1", wo));
  wo.asif_commit_timestamps = {2000};
  ASSERT_OK(Flush());
  ASSERT_OK(Put("a", "1", wo));
  ASSERT_OK(Put("z", "1", wo));
  ASSERT_OK(Put("bar", "1", wo));
  ASSERT_OK(Put("foo", "1", wo));
  ...
  iter = NewIterator(ro);
  iter->Seek("foo");
  // upper bound is `x`, a y1... z, pass x is invalid
  ASSERT_FALSE(iter->Valid());
  delete iter;
}
...
TEST_P(DBIteratorTest, TableFilter_WithTimestamp) {
  ...
  ASSERT_OK(Put("a", "1", wo));
  dbfull()->Flush(FlushOptions());
  wo.asif_commit_timestamps = {100001};
  ASSERT_OK(Put("b", "2", wo));
  ASSERT_OK(Put("c", "3", wo));
  dbfull()->Flush(FlushOptions());
  wo.asif_commit_timestamps = {100002};
  ASSERT_OK(Put("d", "4", wo));
  ASSERT_OK(Put("e", "5", wo));
  ASSERT_OK(Put("f", "6", wo));
  dbfull()->Flush(FlushOptions());
...
TEST_P(DBIteratorTest, UpperBoundWithPrevReseek_WithTimestamp) {
  ...
  DestroyAndReopen(options);
  WriteOptions wo;
  wo.asif_commit_timestamps = {222222};
  // write a bunch of kvs to the database.
  ASSERT_OK(Put("a", "1", wo));
  ASSERT_OK(Put("y", "1", wo));
  ASSERT_OK(Put("z", "1", wo));
  ASSERT_OK(Flush());
  wo.asif_commit_timestamps = {222223};
  ASSERT_OK(Put("a", "1", wo));
  ASSERT_OK(Put("z", "1", wo));
  ASSERT_OK(Put("bar", "1", wo));
  ASSERT_OK(Put("foo", "1", wo));
  ASSERT_OK(Put("foo", "2", wo));

  ASSERT_OK(Put("foo", "3", wo));
  ASSERT_OK(Put("foo", "4", wo));
  ASSERT_OK(Put("foo", "5", wo));
  const Snapshot* snapshot = db_->GetSnapshot();
  wo.asif_commit_timestamps = {222224};
  ASSERT_OK(Put("foo", "6", wo));
...
TEST_P(DBIteratorTest, SkipStatistics_WithTimestamp) {
  ...
  // write a bunch of kvs to the database.
  ASSERT_OK(Put("a", "1", wo));
  ASSERT_OK(Put("b", "1", wo));
  ASSERT_OK(Put("c", "1", wo));
  ASSERT_OK(Flush());
  wo.asif_commit_timestamps = {200};
  ASSERT_OK(Put("d", "1", wo));
  ASSERT_OK(Put("e", "1", wo));
  ASSERT_OK(Put("f", "1", wo));
  ASSERT_OK(Put("a", "2", wo));
  ASSERT_OK(Put("b", "2", wo));
  ASSERT_OK(Flush());
  wo.asif_commit_timestamps = {300};
  ASSERT_OK(db_->Delete(wo, "d"));
  ASSERT_OK(db_->Delete(wo, "e"));
  ASSERT_OK(db_->Delete(wo, "f"));
  ReadOptions ro;
  ro.read_timestamp = 400;
  Iterator* iter = NewIterator(ro);
  int count = 0;
  ...
  ASSERT_OK(Put("aa", "1", wo));
  ASSERT_OK(Put("ab", "1", wo));
  ASSERT_OK(Put("ac", "1", wo));
  ASSERT_OK(Put("ad", "1", wo));
  ASSERT_OK(Flush());
  ASSERT_OK(db_->Delete(wo, "ab"));
  ASSERT_OK(db_->Delete(wo, "ac"));
  ASSERT_OK(db_->Delete(wo, "ad"));
...
TEST_P(DBIteratorTest, SeekAfterHittingManyInternalKeys_WithTimestamp) {
  ...
  ropts.read_timestamp = 999;
  unique_ptr<Iterator> iter3(NewIterator(ropts));
